...
// Открытие замка
// В текщей версии мы отключаем питание от сервопривода, пока он не используется, т.к. дверь находится рядом с кроватью и от сервопривода идет неприятный писк, пока на него подается напряжение.
// Поэтому пока он не используется, мы отключаем питание, а как нужно его использовать, подаем на время и отключаем снова.
void servoOpen() {
